...
# Meta
/*
Name: Ritchey Markup To Page Based HTML i1 v1
...
*/
# Content
if (function_exists('ritchey_markup_to_page_based_html_i1_v1') === FALSE){
function ritchey_markup_to_page_based_html_i1_v1($source_file, $destination_file, $css_file = NULL, $preserve_empty_lines = NULL, $overwrite = NULL, $display_errors = NULL){
	$errors = array();
	...
	if ($css_file === NULL){
		$css_file = "{$location}/assets/general-theme-v4.css";
...
HTML2;
		...
		unset($value);
		### Add Page HTML, CSS, & Javascript
		$data = implode(PHP_EOL, $data);
		// Top HTML + CSS (this helps create a valid HTML page, and makes pages hide overflow)
		$css = file_get_contents($css_file);
		$title = hash('sha3-256', $data);
		$top_html = <<<HEREDOC
<!DOCTYPE html>
<html>
<head>
...
<body>
HEREDOC;
		// Javascript (this makes page overflows noticeable)
		$javascript_content = <<<HEREDOC
<script>
console.log('Javascript is running!');
function flag_overflow_v1() {
  var elements = document.querySelectorAll('.page_outter');
  elements.forEach(el => {
    // Clear overflow classes
    el.classList.remove('overflow_bottom', 'overflow_right');
    // Check if overflows horizontally
    if (el.scrollWidth > el.clientWidth) {
      el.classList.add('overflow_right');
    }
    // Check if overflows vertically
    if (el.scrollHeight > el.clientHeight) {
      el.classList.add('overflow_bottom');
    }
  });
}
// Run on page load
window.addEventListener('DOMContentLoaded', flag_overflow_v1);
// Run again on resize
window.addEventListener('resize', flag_overflow_v1);
</script>
HEREDOC;	
		$data = $data . PHP_EOL . $javascript_content;
...
HEREDOC;
		### Write data to file
		$data = $top_html . PHP_EOL . $data . PHP_EOL . $bottom_html;
	...
	if ($display_errors === TRUE){
		if (@empty($errors) === FALSE){
			...
			if (function_exists('ritchey_markup_to_page_based_html_i1_v1_format_error') === FALSE){
				function ritchey_markup_to_page_based_html_i1_v1_format_error($errno, $errstr){
					echo $errstr;
				}
			}
			set_error_handler("ritchey_markup_to_page_based_html_i1_v1_format_error");
